...
class ProfessorSystem {
  ...
  // ---------- публичный showMessage ----------
  showMessage(text, emotion = 'default', duration = 3000, state = 'idle') {
    if (!this.containerEl) return this;
  ...
  // Остальные методы (showGreeting, onCorrect и т.д.) уже были,
  // главное – наличие showMessage
  // ---------- остальные методы (сокращённо, они у вас уже есть) ----------
  showGreeting() {
    ...
    this.showMessage(text, 'default', 5000, 'greeting');
  ...
  onCorrect(topicKey) {
    ...
    this.showMessage(text, 'happy', 4000, 'success');
    return this;
  }

  onWrong(topicKey, correctAnswerText) {
    const text = this.getPersonalizedMessage('wrong', correctAnswerText);
    this.showMessage(text, 'sad', 5000, 'error');
    return this;
  }

  showExplanation(text) {
    this.showMessage(text, 'hint', 6000, 'hint');
    return this;
  }

  showHint(text) {
    this.showMessage('💡 ' + text, 'hint', 5000, 'hint');
    return this;
  }
  ...
  onLevelUp(level) {
    ...
    this.showMessage(text, 'happy', 5000, 'levelUp');
    return this;
  }
  ...
  onStreak(days) {
    ...
    this.showMessage(text, 'happy', 5000, 'streak');
    return this;
  }

  onChestOpen() {
    const text = this._randomFromArray(this.messages.chest);
    this.showMessage(text, 'happy', 6000, 'chest');
    return this;
  }
  ...
  onTopicComplete(topicTitle) {
    ...
    this.showMessage(text, 'happy', 5500, 'topicComplete');
    return this;
  }

  onDailyTaskDone() {
    const text = this._randomFromArray(this.messages.dailyTaskDone);
    this.showMessage(text, 'happy', 4000, 'dailyTask');
    return this;
  }

  onAchievement(text) {
    this.showMessage(text || '🏆 Новое достижение!', 'happy', 5000, 'achievement');
    return this;
  }
  ...
  showFiveCorrectMessage() {
    ...
    this.showMessage(msg, 'happy', 5000, 'success');
    return this;
  }

  showDiagnosis(skill) {
    const msg = 'Похоже, нужно подтянуть навык «' + skill + '». Давай уделим ему внимание?';
    this.showMessage(msg, 'hint', 5000, 'diagnosis');
    return this;
  }
  ...
  trackAnswer(isCorrect) {
    ...
    if (flags.fiveCorrect) this.consecutiveCorrect = 0;
    if (flags.threeErrors) this.consecutiveErrors = 0;
  ...
  // Очередь с кнопкой (используется для "Изучить теорию")
  ...
  _showMessageInternal(text, emotion, duration, state, onComplete, button) {
    if (!this.containerEl) { onComplete(); return; }
    this._setState(state, duration);
    this.bubbleEl.textContent = text;
    this.bubbleEl.classList.add('visible');

    const src = this.images[emotion] || this.images.default;
    if (src) this.characterEl.src = src;
    this.characterEl.classList.add('prof-leaning');
