...
(function addChestStyles() {
  ...
  style.textContent = `
    ...
    .modal-card {
      ...
      max-width: 320px; width: 90%; box-shadow: 0 12px 40px rgba(0,0,0,0.5);
      animation: scaleIn .25s cubic-bezier(.34,1.3,.64,1);
    }
    .modal-title { font-family: var(--font-h); font-size: 18px; font-weight: 800; margin-bottom: 8px; }
    .modal-sub { font-size: 14px; color: var(--muted); margin-bottom: 12px; }
    .modal-reward { font-size: 28px; font-weight: 800; color: var(--gold); margin: 16px 0; }
    .modal-btn {
...
function openChest() {
  ...
  showChestOpeningAnimation(function() {
    ...
    applyReward(reward);
    // Реакция профессора на сундук
    professor.onChestOpen();
...
function applyReward(reward) {
  switch(reward.type) {
    case 'xp': addXP(reward.value); break;
    case 'streak_day': showToast('🎁 +1 день серии (будет доступно позже)'); break;
    case 'boost_x2':
