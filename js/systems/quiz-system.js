...
// Счётчики для персонализации профессора
...
async function requestAIExplanation(isCorrect, qText, topic) {
  ...
  var prompt = isCorrect
    ? 'Ты Профессор Гео. Ученик правильно ответил на вопрос ОГЭ: «' + qText + '». Объясни кратко (2-3 предложения), почему этот ответ правильный.'
    : 'Ты Профессор Гео. Ученик ошибся в вопросе ОГЭ: «' + qText + '». Дай КОРОТКОЕ объяснение правильного ответа, укажи ключевое правило.';
...
function showResult() {
  ...
  // ★ ИСПРАВЛЕНИЕ: Сохраняем пройденный урок перед показом результатов
