// ==========================================
//  МИКРОУРОКИ (расширенная версия: image_lesson, image_hotspot)
// ==========================================
...
function startTheoryCards(theoryInfo, data, topicKey) {
  ...
  if (data && Array.isArray(data.cards)) {
    ...
    // Старый формат (массив без поля cards) – сразу квиз
...
function renderMicroStep() {
  ...
  for (var i = 0; i < total; i++) {
    ...
    var s = microSteps[i];
    if (s.type === 'quiz' || s.type === 'true_false' || s.type === 'choose_image' || s.type === 'image_hotspot') dotClass += ' quiz';
  ...
  if (step.type === 'lesson') {
    ...
    html += '<img src="' + step.image + '" style="width:100%;border-radius:12px;" id="hotspot-img" />';
